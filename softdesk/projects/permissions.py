...
class CanModifyContributors(BasePermission):
    message = (
        "Sorry, you don't have permission to access "
        "the collaborators informations."
        "You're not the author of this project"
    )
    ...
    def has_object_permission(self, request, view, obj):
        return (
            ...
            == Projects.objects.get(
                id=request.parser_context["kwargs"]["project_pk"]
            ).author_user_id
        )
        ...
        try:
            ...
            raise NotFound(
                detail=f"Sorry, project "
                       f"{request.parser_context['kwargs']['project_pk']}"
                       f" doesn't exist"
            )
        return obj.project_id.id == int(
...
class IsIssueCommentsInThisProject(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        return Issues.objects.get(
            id=int(request.parser_context["kwargs"]["issue_pk"])
        ).project_id == Projects.objects.get(
            id=int(request.parser_context["kwargs"]["project_pk"])
        )
...
class IsCommentInThisIssue(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        return obj.issue_id.id == int(
            request.parser_context["kwargs"]["issue_pk"])
